  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col h-full">
        <SheetHeader>
          <SheetTitle className="font-serif text-2xl">Tu bolso</SheetTitle>
          <SheetDescription>
            {totalItems === 0 ? "Aún no has añadido nada" : `${totalItems} artículo${totalItems !== 1 ? "s" : ""}`}
          </SheetDescription>
        </SheetHeader>
          ...
          {items.length === 0 ? (
            <div className="flex-1 flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Tu bolso está vacío</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="space-y-4 pt-4 border-t">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-semibold">{items[0]?.price.currencyCode} {totalPrice.toFixed(2)}</span>
                </div>
